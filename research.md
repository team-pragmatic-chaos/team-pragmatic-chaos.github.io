...
Let's take the same example as above. Suppose we generate the number `18` and we are using an interval of `4` frames, then the frames `18, 22, 26, 30` are selected as input for video-1 in the current batch. 
